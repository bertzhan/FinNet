-- 迁移：将 hs_listed_companies 的 established_date、listed_date 从 BIGINT（毫秒时间戳）改为 DATE
-- 日期：2025-02-21
-- 描述：存储为实际日期值（YYYY-MM-DD），便于查询和展示，与 hk_listed_companies 一致

BEGIN;

-- 1. established_date: BIGINT(ms) -> DATE
ALTER TABLE hs_listed_companies
...
-- 3. 更新注释
COMMENT ON COLUMN hs_listed_companies.established_date IS '成立日期';
COMMENT ON COLUMN hs_listed_companies.listed_date IS '上市日期';
