...
def test_fulltext_retrieval(company_code: str, company_name: str, year: int, quarter: int, doc_type: str, period: str) -> Dict[str, Any]:
    """测试全文检索接口"""
    test_name = "POST /api/v1/retrieval/fulltext"
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/v1/retrieval/fulltext",
            json=payload,
            timeout=30
        )
...
def run_tests() -> List[Dict[str, Any]]:
    ...
    print("=" * 60)
    print("测试全文检索接口: POST /api/v1/retrieval/fulltext")
    print("=" * 60)
    ...
    for company in TEST_COMPANIES:
        for test_case in RETRIEVAL_TEST_CASES:
            ...
            result = test_fulltext_retrieval(
                company['code'],
                company['name'],
                TEST_YEAR,
                test_case['quarter'],
