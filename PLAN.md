...
│  │                         存储引擎                                         │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌────────────┐   │ │
│  │  │ MinIO    │ │ Iceberg  │ │PostgreSQL│ │ Milvus   │ │Neo4j ││   │ │
...
| 图数据库 | Neo4j | - | 成熟稳定、Cypher查询语言、生态完善 |
| 全文搜索引擎 | Elasticsearch | - | 全文检索、聚合分析、实时搜索、生态完善 |
...
Neo4j 用于存储文档和分块的图结构，支持文档检索和图遍历查询。

**节点类型：**

1. **Document 节点**
   - 属性：`id`, `stock_code`, `company_name`, `market`, `doc_type`, `year`, `quarter`
   - 唯一约束：`id`

2. **Chunk 节点**
   - 属性：`id`, `document_id`, `chunk_index`, `chunk_text`, `title`, `title_level`, `chunk_size`, `is_table`
   - 唯一约束：`id`

**关系类型：**

1. **BELONGS_TO**：`Chunk -[:BELONGS_TO]-> Document`
   - 表示分块属于文档
   - 仅顶级分块（`parent_chunk_id` 为 `None`）直接连接到文档

2. **HAS_CHILD**：`Chunk -[:HAS_CHILD]-> Chunk`
   - 表示父分块包含子分块
   - 方向：从父分块指向子分块
...
PostgreSQL (DocumentChunk) 
    ↓
GraphBuilder.build_document_chunk_graph()
    ↓
Neo4j (Document/Chunk 节点 + 关系)
```
...
NEO4J_USER=neo4j
NEO4J_PASSWORD=finnet123456
NEO4J_DATABASE=neo4j
```

**相关代码文件：**

- `src/storage/graph/neo4j_client.py` - Neo4j 客户端封装
- `src/processing/graph/graph_builder.py` - 图构建服务
- `src/processing/compute/dagster/jobs/graph_jobs.py` - 图构建 Dagster 作业
- `src/common/config.py` - Neo4j 配置（`Neo4jConfig`）

**使用场景：**

- **文档检索**：通过文档节点快速查找相关分块
- **层级遍历**：通过 `HAS_CHILD` 关系遍历分块层级结构
- **图查询**：使用 Cypher 查询语言进行复杂的图遍历和关系分析
- **GraphRAG**：支持基于图结构的检索增强生成
...
#### 4.1.5 全文搜索引擎：Elasticsearch

**选型理由：**
- 成熟的全文搜索引擎，支持复杂的查询和聚合分析
- 支持中文分词（IK Analyzer、Smart Chinese Analyzer）
- 实时索引更新，支持高并发查询
- 丰富的 RESTful API，易于集成
- 支持分布式部署，可水平扩展
- 生态完善，与 Kibana 集成提供可视化分析

**使用场景：**
- **全文检索**：文档内容、公告文本的全文搜索
- **结构化查询**：基于元数据（股票代码、日期、文档类型）的精确查询
- **聚合分析**：按市场、行业、时间维度进行统计分析
- **实时搜索**：支持实时数据更新和查询
- **混合检索**：与向量检索（Milvus）结合，提供混合检索能力

**索引设计：**

1. **文档索引（documents）**
   - 字段：`id`, `stock_code`, `company_name`, `market`, `doc_type`, `publish_date`, `title`, `content`, `summary`
   - 分词：`content` 和 `title` 使用中文分词器
   - 元数据：`stock_code`, `market`, `doc_type` 作为 keyword 类型，支持精确匹配和聚合

2. **分块索引（chunks）**
   - 字段：`id`, `document_id`, `chunk_index`, `chunk_text`, `title`, `title_level`, `chunk_size`, `is_table`
   - 关联：通过 `document_id` 关联到文档索引
   - 支持：分块级别的全文检索和过滤
...
- **Elasticsearch**：负责全文检索、结构化查询、聚合分析
- **Milvus**：负责向量相似度检索、语义搜索
- **混合检索**：先通过 Elasticsearch 进行关键词/元数据过滤，再通过 Milvus 进行向量相似度排序
...
**全文检索存储：**
- 存储引擎：Elasticsearch
- 索引类型：文档索引（documents）、分块索引（chunks）
- 支持中文分词：IK Analyzer、Smart Chinese Analyzer
- 支持结构化查询：基于元数据（股票代码、市场、文档类型、日期）的精确匹配和聚合分析

**混合检索策略：**
- **关键词检索**：使用 Elasticsearch 进行全文检索和元数据过滤
- **语义检索**：使用 Milvus 进行向量相似度检索
- **混合检索**：先通过 Elasticsearch 进行关键词/元数据过滤，再通过 Milvus 进行向量相似度排序，结合两种检索结果
...
| Neo4j | 8核 | 32GB | 500GB SSD | - | 1-3 |
| Elasticsearch | 8核 | 16GB | 500GB SSD | - | 1（可扩展） |
