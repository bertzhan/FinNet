# -*- coding: utf-8 -*-
"""
文档查询接口测试
根据stock_code, year, quarter, doc_type查询document_id
"""

import requests
import json
import sys
from typing import Optional


BASE_URL = "http://localhost:8000"


def test_document_query(
    stock_code: str,
    year: int,
    quarter: Optional[int],
    doc_type: str
) -> bool:
    """
    测试文档查询接口
    
    Args:
        stock_code: 股票代码
        year: 年份
        quarter: 季度（可选）
        doc_type: 文档类型
        
    Returns:
        是否测试成功
    """
    print(f"\n{'='*60}")
    print(f"测试文档查询接口")
    print(f"{'='*60}")
    print(f"股票代码: {stock_code}")
    print(f"年份: {year}")
    print(f"季度: {quarter if quarter else 'None (年度文档)'}")
    print(f"文档类型: {doc_type}")
    print()
    
    url = f"{BASE_URL}/api/v1/document/query"
    payload = {
        "stock_code": stock_code,
        "year": year,
        "doc_type": doc_type
    }
    
    # 如果quarter不为None，添加到payload
    if quarter is not None:
        payload["quarter"] = quarter
    
    try:
        print("发送请求...")
        response = requests.post(url, json=payload, timeout=30)
        
        # 检查 HTTP 状态码
        if response.status_code != 200:
            print(f"✗ HTTP 错误: {response.status_code}")
            print(f"  响应内容: {response.text[:200]}")
            return False
        
        result = response.json()
        
        # 显示结果
        print(f"✓ 请求成功")
        print()
        print(f"查询结果:")
        print(f"  - 是否找到: {'是' if result.get('found') else '否'}")
        
        document_id = result.get('document_id')
        found = result.get('found', False)
        message = result.get('message')
        
        if document_id:
            print(f"  - 文档ID: {document_id}")
            print(f"  ✅ 找到匹配的文档")
        else:
            print(f"  - 文档ID: null")
        
        if message:
            print(f"\n  提示消息: {message}")
        elif not found:
            print(f"  ⚠️  未找到匹配的文档")
        
        print()
        
        return True
        
    except requests.exceptions.ConnectionError:
        print(f"✗ 连接失败: 无法连接到 API 服务 ({BASE_URL})")
        print(f"  请确保 FinNet API 服务正在运行")
        return False
    except requests.exceptions.Timeout:
        print(f"✗ 请求超时: API 服务响应时间过长")
        return False
    except json.JSONDecodeError as e:
        print(f"✗ JSON 解析失败: {e}")
        print(f"  响应内容: {response.text[:200]}")
        return False
    except Exception as e:
        print(f"✗ 测试失败: {e}")
        import traceback
        traceback.print_exc()
        return False


def test_multiple_cases():
    """测试多个查询案例"""
    print(f"\n{'='*60}")
    print(f"批量测试多个查询案例")
    print(f"{'='*60}")
    
    test_cases = [
        {
            "stock_code": "000001",
            "year": 2024,
            "quarter": 3,
            "doc_type": "quarterly_reports",
            "description": "平安银行2024年第三季度季报"
        },
        {
            "stock_code": "000001",
            "year": 2024,
            "quarter": None,
            "doc_type": "annual_reports",
            "description": "平安银行2024年年报"
        },
        {
            "stock_code": "000001",
            "year": 2024,
            "quarter": 1,
            "doc_type": "quarterly_reports",
            "description": "平安银行2024年第一季度季报"
        },
        {
            "stock_code": "000002",
            "year": 2024,
            "quarter": 2,
            "doc_type": "quarterly_reports",
            "description": "万科A 2024年第二季度季报"
        },
    ]
    
    results = []
    for case in test_cases:
        print(f"\n测试: {case['description']}")
        success = test_document_query(
            stock_code=case["stock_code"],
            year=case["year"],
            quarter=case["quarter"],
            doc_type=case["doc_type"]
        )
        results.append((case["description"], success))
    
    # 汇总结果
    print(f"\n{'='*60}")
    print(f"批量测试结果汇总")
    print(f"{'='*60}")
    passed = sum(1 for _, success in results if success)
    total = len(results)
    
    for description, success in results:
        status = "✓ 通过" if success else "✗ 失败"
        print(f"{status}: {description}")
    
    print(f"\n总计: {passed}/{total} 通过")
    return passed == total


def main():
    """主函数"""
    import argparse
    
    global BASE_URL
    
    parser = argparse.ArgumentParser(description="测试文档查询接口")
    parser.add_argument(
        "--stock-code",
        type=str,
        help="股票代码"
    )
    parser.add_argument(
        "--year",
        type=int,
        help="年份"
    )
    parser.add_argument(
        "--quarter",
        type=int,
        help="季度（1-4），如果不提供则查询年度文档"
    )
    parser.add_argument(
        "--doc-type",
        type=str,
        help="文档类型（如：quarterly_reports, annual_reports等）"
    )
    parser.add_argument(
        "--batch",
        action="store_true",
        help="运行批量测试多个查询案例"
    )
    parser.add_argument(
        "--base-url",
        type=str,
        default=BASE_URL,
        help=f"API 基础 URL（默认: {BASE_URL}）"
    )
    
    args = parser.parse_args()
    
    # 更新 BASE_URL
    BASE_URL = args.base_url
    
    # 检查 API 服务是否可访问
    try:
        health_url = f"{BASE_URL}/health"
        response = requests.get(health_url, timeout=5)
        if response.status_code == 200:
            print(f"✓ API 服务可访问: {BASE_URL}")
        else:
            print(f"⚠️  API 服务响应异常: {response.status_code}")
    except Exception as e:
        print(f"✗ 无法连接到 API 服务: {e}")
        print(f"  请确保 FinNet API 服务正在运行")
        sys.exit(1)
    
    # 运行测试
    if args.batch:
        success = test_multiple_cases()
    elif args.stock_code and args.year and args.doc_type:
        success = test_document_query(
            stock_code=args.stock_code,
            year=args.year,
            quarter=args.quarter,
            doc_type=args.doc_type
        )
    else:
        # 默认测试
        print("未指定完整参数，运行默认测试...")
        success = test_document_query(
            stock_code="000001",
            year=2024,
            quarter=3,
            doc_type="quarterly_reports"
        )
    
    sys.exit(0 if success else 1)


if __name__ == "__main__":
    main()
