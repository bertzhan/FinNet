...
"""
快速测试 Embedder 功能
验证 sentence-transformers 安装和基本功能
"""
...
# 设置环境变量，避免导入时触发 Milvus 相关模块
import os
os.environ.setdefault('SKIP_MILVUS_CHECK', '1')
...
def test_embedder_init():
    ...
    try:
        # 直接导入 bge_embedder，避免导入 vectorizer（需要 pymilvus）
        ...
        # 直接实例化，避免使用 get_embedder（会导入 vectorizer）
...
def main():
    ...
    if passed == total:
        ...
        print()
        print("下一步：")
        print("1. 运行完整测试: python examples/test_vectorize_simple.py")
        print("2. 在 Dagster UI 中运行向量化作业")
    else:
