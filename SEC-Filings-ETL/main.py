...
Examples:
  ...
  python main.py incremental
    ...
    foreign_backfill_parser.add_argument(
        '--tickers',
        type=str,
        help='Comma-separated list of tickers to process (e.g., SPOT,ABEV,SNDL)'
    )
    foreign_backfill_parser.add_argument(
        '--include-6k',
        choices=['off', 'minimal', 'financial', 'all'],
        default='minimal',
        help='6-K inclusion policy: off=exclude all 6-K, minimal=primary doc only, financial=with financials, all=all (default: minimal)'
    )
