...
class Settings(BaseSettings):
    ...
    sec_rps: int = Field(default=10, description="Global rate limit (requests per second) across all processes")
    ...
    sec_cache_ttl_hours: int = Field(default=12, description="Cache TTL for company submissions in hours")
    ...
    html_workers: int = Field(default=8, description="Concurrent workers for HTML downloads")
    image_workers: int = Field(default=4, description="Concurrent workers for image downloads")
    download_chunk_size: int = Field(default=262144, description="Download chunk size in bytes (256KB)")
    batch_size: int = Field(default=50, description="Batch size for DB commits")
