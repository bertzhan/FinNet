...
def create_artifact_records(
    ...
    Args:
        filing_id: Filing ID
        cik: Company CIK (used for correct URL construction)
    ...
    # Primary HTML document - use company CIK, not accession prefix
    from services.sec_api import SECAPIClient
    sec_client = SECAPIClient(use_cache=False)  # No cache needed for URL construction
    primary_url = sec_client.construct_primary_html_url(cik, accession, primary_document)

    artifacts.append({
        'filing_id': filing_id,
        'artifact_type': ARTIFACT_TYPE_HTML,
        'filename': primary_document if primary_document else 'index.html',
        'url': primary_url,
...
def should_include_6k_filing(
    ...
    Args:
        ...
        include_policy: "off", "minimal", "financial", or "all"
    ...
    if include_policy == "off":
        # Exclude all 6-K filings
        return {'include': False, 'include_exhibits': False}

    elif include_policy == "minimal":
...
class ForeignBackfillJob:
    ...
    def __init__(
        ...
        Args:
            ...
            include_6k: 6-K inclusion policy ("off", "minimal", "financial", "all")
        ...
        self.tickers = [t.strip().upper() for t in tickers.split(',')] if tickers else None
    ...
    def _process_companies(self, session, run: ExecutionRun) -> Dict:
        ...
        if self.tickers:
            query = query.filter(Company.ticker.in_(self.tickers))
        ...
        stats = {
            ...
            'filings_skipped_no_primary_doc': 0,
            'artifacts_created': 0,
            'errors': 0,
            'form_counts': {
                '20-F': 0,
            ...
            try:
                ...
                for filing_data in filings:
                    ...
                    if not filing_data.get('primary_document'):
                        ...
                        stats['filings_skipped_no_primary_doc'] += 1
