...
**V2.1 采用 Software-Defined Assets**：
- 使用 `@asset` 装饰器定义数据资产
- 通过 `deps` 参数声明资产依赖关系
...
```mermaid
flowchart TB
    subgraph Ingestion [Ingestion 层]
        HSCompanies[hs_listed_companies]
        HKCompanies[hk_listed_companies]
        USCompanies[us_listed_companies]
        HSCrawl[crawl_hs_reports<br/>crawl_hs_ipo]
        HKCrawl[crawl_hk_reports]
        USCrawl[crawl_us_reports]
        HSCompanies --> HSCrawl
        HKCompanies --> HKCrawl
        USCompanies --> USCrawl
    end
    ...
    subgraph Bronze [Bronze 层]
        BronzeHS[ bronze/hs_stock ]
        BronzeHK[ bronze/hk_stock ]
        BronzeUS[ bronze/us_stock ]
    end
...
### 辅图：Processing 流水线（简化）

```mermaid
flowchart LR
    Bronze[Bronze<br/>原始文档] --> Parse[doc_parse_op<br/>parsed_documents]
    Parse --> Chunk[doc_chunk_op<br/>chunked_documents]
    Chunk --> V[doc_vectorize_op<br/>vectorized_chunks]
    Chunk --> G[doc_toc_graph_op<br/>doc_toc_graph]
    Chunk --> E[doc_index_op<br/>doc_index]
```

**关键依赖关系说明**:

- **Ingestion**: 公司列表（hs_listed_companies / hk_listed_companies / us_listed_companies）→ 爬虫 → Bronze
- `doc_parse_op` 依赖 crawl (bronze → silver/parsed_documents)
- `doc_chunk_op` 依赖 `doc_parse_op` (silver/parsed_documents → silver/chunked_documents)
- `doc_vectorize_op`、`doc_toc_graph_op`、`doc_index_op` **直接依赖** `doc_chunk_op` (silver/chunked_documents → gold)
- **Gold 层**包含: vectorized_chunks、doc_toc_graph、doc_index（三者均依赖 chunked_documents，并行执行）
...
# 港股
["bronze", "hk_stock", "annual_report", "2023", "Q4"]

# 美股 10-K
["bronze", "us_stock", "10k", "2023"]
```
...
# 图节点（doc_toc_graph_op 产出）
["gold", "doc_toc_graph", "hs_stock", "annual_report", "000001"]

# Elasticsearch 索引（doc_index_op 产出）
...
| `gold_vectorized_chunks` | Gold | silver_chunked_documents | 向量化分块 |
| `gold_doc_toc_graph` | Gold | silver_chunked_documents | Neo4j 图节点（doc_toc_graph_op） |
| `gold_doc_index` | Gold | silver_chunked_documents | ES 索引（doc_index_op） |
...
**资产key格式**:
```python
["bronze", "hs_stock", doc_type_str, str(year), f"Q{quarter}"]  # A股
["bronze", "hk_stock", doc_type_str, str(year), f"Q{quarter}"]  # 港股
["bronze", "us_stock", doc_type_str, str(year)]  # 美股
["quality_metrics", "crawl_validation"]
```
...
**资产key格式**:
```python
["gold", "vectorized_chunks", market, doc_type, stock_code]
["quality_metrics", "vectorize_validation"]
```
...
**资产key格式**:
```python
["gold", "doc_toc_graph", market, doc_type, stock_code]
["quality_metrics", "graph_validation"]
```
...
**资产key格式**:
```python
["gold", "doc_index", market, doc_type, stock_code]
["quality_metrics", "elasticsearch_validation"]
```
...
1. **Bronze层**: `["bronze", "hs_stock", "annual_report", "2023", "Q4"]`
   - 由 `crawl_hs_reports_op` 创建

2. **Silver层 - 解析**: `["silver", "parsed_documents", "hs_stock", "annual_report", "000001", "2023", "Q4"]`
   - 由 `doc_parse_op` 创建
   - parent_asset_key: `bronze/hs_stock/annual_report/2023/Q4`

3. **Silver层 - 分块**: `["silver", "chunked_documents", "hs_stock", "annual_report", "000001"]`
   - 由 `doc_chunk_op` 创建
   - parent_asset_key: `silver/parsed_documents/hs_stock/annual_report/000001/2023/Q4`

4. **Gold层 - 向量化**: `["gold", "vectorized_chunks", "hs_stock", "annual_report", "000001"]`
   - 由 `doc_vectorize_op` 创建
   - parent_asset_key: `silver/chunked_documents/hs_stock/annual_report/000001`

5. **Gold层 - 图**: `["gold", "doc_toc_graph", "hs_stock", "annual_report", "000001"]`
   - 由 `doc_toc_graph_op` 创建
   - parent_asset_key: `silver/chunked_documents/hs_stock/annual_report/000001`

6. **Gold层 - 索引**: `["gold", "doc_index", "hs_stock", "annual_report", "000001"]`
   - 由 `doc_index_op` 创建
   - parent_asset_key: `silver/chunked_documents/hs_stock/annual_report/000001`
...
# 查询资产的所有物化事件
asset_key = AssetKey(["silver", "chunked_documents", "hs_stock", "annual_report", "000001"])
...
- ✅ **数据血缘图重设计**：多市场（hs_stock/hk_stock/us_stock）、Ingestion 层、公司列表上游
- ✅ **vectorized_chunks 归 Gold 层**：与 doc_toc_graph、doc_index 并列
- ✅ **资产更名**：graph_nodes → doc_toc_graph，elasticsearch_index → doc_index（对齐 op 名称）
- ✅ 移除 mermaid style  directive，兼容暗色主题
- ✅ 资产示例统一为 hs_stock/hk_stock/us_stock
