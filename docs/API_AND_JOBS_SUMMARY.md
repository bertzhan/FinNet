...
| Job 名称 | 说明 | 调度 | 传感器 |
|---------|------|------|--------|
| `crawl_hs_reports_job` | A股定期报告爬取 | 每天执行 | `manual_trigger_reports_sensor` |
| `crawl_hs_ipo_job` | A股IPO招股书爬取 | 每天执行 | `manual_trigger_ipo_sensor` |

### 数据处理 Jobs

| Job 名称 | 说明 | 调度 | 传感器 |
|---------|------|------|--------|
| `doc_parse_job` | PDF 解析作业 | 每小时/每天 | `manual_trigger_parse_sensor` |
| `doc_chunk_job` | 文本分块作业 | 每小时/每天 | `manual_trigger_chunk_sensor` |
| `doc_vectorize_job` | 向量化作业 | 每2小时/每天 | `manual_trigger_vectorize_sensor` |

### 索引和构建 Jobs

| Job 名称 | 说明 | 调度 | 传感器 |
|---------|------|------|--------|
| `doc_index_job` | Elasticsearch 索引作业 | 每小时/每天 | `manual_trigger_elasticsearch_sensor` |
| `doc_toc_graph_job` | 图构建作业 | 每小时/每天 | `manual_trigger_graph_sensor` |

### 数据更新 Jobs

| Job 名称 | 说明 | 调度 | 传感器 |
|---------|------|------|--------|
| `get_hs_companies_job` | 上市公司列表更新作业 | 每天执行 | `manual_trigger_companies_sensor` |
...
```
1. 数据采集 (Crawl Jobs)
   ├─ crawl_hs_reports_job → PDF 文件
   └─ crawl_hs_ipo_job → PDF 文件
...
4. 向量化 (Vectorize Job) [并行]
   └─ doc_vectorize_job → 向量嵌入
   
5. 全文索引 (Elasticsearch Job) [并行]
   └─ doc_index_job → Elasticsearch 索引
   
6. 图构建 (Graph Job) [并行]
   └─ doc_toc_graph_job → Neo4j 图数据库
   
7. 数据更新 (Company List Job) [独立]
   └─ get_hs_companies_job → 上市公司列表
```
